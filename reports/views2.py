...
def RPTGen(datafiles, surveyor, inspectiontime, company, Address,
           ...
           room_pictures, Image_logo, comment, popup=True):
    # global DATA
    ...
    def section4(room_data: RoomData, room_pictures):
        ...
        pdf.set_font('Arial', '', FontSize.CONTENT)
          ...
          # Dynamically add images for the current room
    if room_pictures:
        pdf.add_page()
        pdf.set_font('Arial', 'B', FontSize.CONTENT)
        pdf.cell(160, 8, 'Room Images', 0, 1, 'C', True)
        pdf.ln(5)

        # Define image placement constants
        IMAGE_WIDTH = 80
        IMAGE_HEIGHT = 60
        SPACE_BETWEEN = 10
        X_OFFSET_LEFT = 20
        X_OFFSET_RIGHT = 110
        Y_OFFSET_TOP = 40
        current_y = Y_OFFSET_TOP

        # Loop through the room images
        for i, image_path in enumerate(room_pictures):
            if i % 2 == 0:  # Left column
                pdf.image(image_path, X_OFFSET_LEFT, current_y, IMAGE_WIDTH, IMAGE_HEIGHT)
                pdf.set_xy(X_OFFSET_LEFT, current_y + IMAGE_HEIGHT + 2)
                pdf.multi_cell(IMAGE_WIDTH, 5, f'Room Image {i + 1}', 0, 'C')
            else:  # Right column
                pdf.image(image_path, X_OFFSET_RIGHT, current_y, IMAGE_WIDTH, IMAGE_HEIGHT)
                pdf.set_xy(X_OFFSET_RIGHT, current_y + IMAGE_HEIGHT + 2)
                pdf.multi_cell(IMAGE_WIDTH, 5, f'Room Image {i + 1}', 0, 'C')
                current_y += IMAGE_HEIGHT + SPACE_BETWEEN

            # Start a new page if current Y exceeds the page height limit
            if current_y + IMAGE_HEIGHT > 250:
                pdf.add_page()
                pdf.set_xy(20, Y_OFFSET_TOP)
                current_y = Y_OFFSET_TOP
